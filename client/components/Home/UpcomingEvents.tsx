            ...
            <div className="flex flex-col gap-2 min-h-[300px]">
              {loading ? (
                Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} className="flex gap-4 items-start p-3 rounded-lg border border-transparent animate-pulse">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 lg:gap-8 h-full">
              {loading ? (
                Array.from({ length: 2 }).map((_, i) => (
                  <article key={i} className="group flex flex-col overflow-hidden bg-white shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07)] border border-gray-100 animate-pulse">
